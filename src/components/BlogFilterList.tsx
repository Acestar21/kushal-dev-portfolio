...
export default function BlogFilterList({ posts }: { posts: Post[] }) {
  ...
  const allTags = useMemo(() => {
    ...
    posts.forEach((post) =>
      post.frontmatter.tags.forEach((tag) => tagCounts.set(tag, (tagCounts.get(tag) ?? 0) + 1))
    );
  ...
  const filtered =
    activeTag === "All" ? posts : posts.filter((post) => post.frontmatter.tags.includes(activeTag));
      ...
      <div className={styles.filters}>
        ...
        {moreTags.length > 0 && (
          <div className={styles.dropdownWrapper} ref={dropdownRef}>
            ...
            >
              {moreTags.includes(activeTag) ? activeTag : "More filters"} ▾
            </button>
