...
export default function SkillsGrid() {
	...
	const domainsWithSkills = useMemo(
		() =>
			ALL_DOMAINS.filter((domain) =>
				skills.some((s) => s.domain.includes(domain)),
			),
		[],
	);
	...
	const filtered =
		activeDomain === "All"
			? skills
			: skills.filter((s) => s.domain.includes(activeDomain));
	...
	return (
		<div>
			<div className={styles.filters}>
				...
				{moreDomains.length > 0 && (
					<div className={styles.dropdownWrapper} ref={dropdownRef}>
						...
						>
							{moreDomains.includes(activeDomain as SkillDomain)
								? activeDomain
								: "More"}{" "}
							▾
						</button>
			...
			<div className={styles.legend}>
				<span
					className={`${styles.legendSwatch} ${styles.tierComfortable}`}
				/>
				Comfortable
				<span
					className={`${styles.legendSwatch} ${styles.tierWorking}`}
				/>
				Working knowledge
				<span
					className={`${styles.legendSwatch} ${styles.tierLearning}`}
				/>
				Currently learning
			</div>
			...
			<div className={styles.grid}>
				{filtered.map((skill) => {
					...
					return (
						<div
							key={skill.name}
							className={`${styles.iconTile} ${tierClass[skill.tier]}`}
						>
							<Icon
								className={styles.icon}
								style={{
									color:
										skill.color ?? "var(--text-secondary)",
								}}
							/>
