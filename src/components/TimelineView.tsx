...
function TimelineNode({
			...
			>
				<div className={styles.minorNode}>
					<button
						...
						onFocus={() =>
							setActiveId(`${entry.date}-${entry.title}`)
						}
						onBlur={() =>
							setActiveId((current) =>
								current === `${entry.date}-${entry.title}`
									? null
									: current,
							)
						}
					...
					>
						<span
							className={`${styles.badge} ${categoryClasses[entry.category]}`}
						>
						...
						<div className={styles.minorPopoverBody}>
							...
							<p className={styles.minorDescription}>
								{entry.description}
							</p>
						</div>
					</div>
	...
	return (
		...
		>
			<div
				className={`${styles.markerWrap} ${isMajor ? styles.majorMarkerWrap : styles.minorMarkerWrap}`}
			>
				<span
					className={`${styles.marker} ${styles.majorMarker}`}
					aria-hidden="true"
				/>
				{isMajor && (
			...
			<div
				className={`${styles.contentWrap} ${isMajor ? styles.majorContentWrap : styles.minorContentWrap}`}
			>
				<div
					className={`${styles.card} ${isMajor ? styles.majorCard : styles.minorCard}`}
				>
...
export default function TimelineView() {
	const [activeId, setActiveId] = useState<string | null>(
		timelineEntries[0]
			? `${timelineEntries[0].date}-${timelineEntries[0].title}`
			: null,
	);
